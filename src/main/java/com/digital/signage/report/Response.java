...
public class Response<T> {
    ...
    public static <F> Response<F>	createInternalServerErrorResponseFromException(Exception e) {
        ...
        r.setName("InternalServerError"); return r; }

    /*
     * public static Response<Void>
     * createInternalServerErrorVoidResponseFromException(Exception e) {
    ...
    /*
     * public static <F> Response<F> createBadRequestResponse(Message message,
     * String messageCode) { Response<F> r = new Response<>();
     * r.setHttpStatusCode(HttpStatus.BAD_REQUEST.value());
     * r.setMessage(message.get(messageCode));
     * r.setCode(HttpStatus.BAD_REQUEST.value()); r.setName("BadRequest"); return r;
     * }
     *
     * public static <F> Response<F> createNotFoundRequestResponse(Message message,
     * String messageCode) { Response<F> r = new Response<>();
     * r.setHttpStatusCode(HttpStatus.NOT_FOUND.value());
     * r.setMessage(message.get(messageCode));
     * r.setCode(HttpStatus.NOT_FOUND.value()); r.setName("NotFound"); return r; }
     */
    ...
    /*
     * public static <F> Response<F> notSupported(Message message, String
     * messageCode) { Response<F> r = new Response<>();
     * r.setHttpStatusCode(HttpStatus.FORBIDDEN.value());
     * r.setMessage(message.get(messageCode));
     * r.setCode(HttpStatus.FORBIDDEN.value()); r.setName("NotSupported"); return r;
     * }
     */
     ...
     * Response<F> r = new Response<>();
     * r.setHttpStatusCode(HttpStatus.FORBIDDEN.value());
     * r.setMessage(message.get(Message.CustomerTiers.
     * FEATURE_NOT_SUPPORTED_FOR_BASIC_CUSTOMER));
     * r.setCode(HttpStatus.FORBIDDEN.value());
     * r.setName("NotSupportedInBasicCustomer"); return r; }
     */
    ...
    /*
     * public static Response<?> parse(String string, Integer httpStatusCode,
     * ObjectMapper objectMapper) throws IOException { Response<?> response =
     * objectMapper.readValue(string,
     * JacksonUtilsKt.getTypeRefOfResponseOfOutAny());
     * response.setHttpStatusCode(httpStatusCode); return response; }
     *
     * public static Response<?> parse( InputStream inputStream, Integer
     * httpStatusCode, ObjectMapper objectMapper ) throws IOException {
     * objectMapper.configure(JsonParser.Feature.AUTO_CLOSE_SOURCE, true);
     * Response<?> response = objectMapper.readValue(inputStream,
     * JacksonUtilsKt.getTypeRefOfResponseOfOutAny());
     * response.setHttpStatusCode(httpStatusCode); return response; }
     */
     ...
     * public FieldMessageBuilder(List<FieldMessage> fieldMessages) { if
     * (fieldMessages.isEmpty()) { throw new
     * IllegalArgumentException("Messages list cannot be empty"); }
     * result(fieldMessages); init(); }
     *
     * public FieldMessageBuilder(FieldMessageHelper fieldMessageHelper) { if
     * (fieldMessageHelper.get().isEmpty()) { throw new
     * IllegalArgumentException("Messages list cannot be empty"); }
     * result(fieldMessageHelper.get()); init(); }
     *
     * public FieldMessageBuilder() { result(new ArrayList<>()); init(); }
     *
     * private void init() { code(400); name("BadRequest"); message(null); }
     *
     * public FieldMessageBuilder addFieldMessage(String field, String message) {
     * getResult().add(new FieldMessage(field, message)); return this; }
     *
     * public FieldMessageBuilder addFieldMessage(FieldMessage fieldMessage) {
     * getResult().add(fieldMessage); return this; } }
     */
